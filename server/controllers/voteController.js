...
module.exports = {
  getVote: (req, res, next) => {
    ...
    db.query(query, values)
      .then(response => {
        console.log('response.rows: ', response.rows[0].sum);
        res.locals.count = response.rows[0].sum;
        return next();
      })
      .catch(err => next(err));
  },
  ...
  upVote: (req, res, next) => {
    if (res.locals.alreadyVoted) {
      ...
      const values = [res.locals.cocktailId, req.body.userId, 1];
      db.query(query, values)
        .then(next())
        .catch(err => next(err))
    } else {
      ...
      const values = [req.body.userId, res.locals.cocktailId, 1]
      db.query(query, values)
        .then(next())
        .catch(err => next(err))
    }
